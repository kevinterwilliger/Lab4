...
public class Colosseum {
    ...
    public static Pokemon buildPokemon() {
        ...
        if (tempPokemon.attackLevel > MAX_ATTACK_LEVEL) {
            while (tempPokemon.attackLevel > MAX_ATTACK_LEVEL) {
                System.out.println("Sorry. The attack level must be between 1 and 49: ");
                tempPokemon.attackLevel = myScan.nextInt();
            }
        }
        System.out.println("Enter your defense level (1-" + (MAX_ATTACK_LEVEL + 1 - tempPokemon.attackLevel) + "):");
        tempPokemon.defenseLevel = myScan.nextInt();
        if (tempPokemon.defenseLevel > (MAX_ATTACK_LEVEL + 1 - tempPokemon.attackLevel)) {
            while (tempPokemon.defenseLevel > (MAX_ATTACK_LEVEL + 1 - tempPokemon.attackLevel)) {
                System.out.println("Sorry. The defense level must be between 1 and " + (MAX_ATTACK_LEVEL + 1 - tempPokemon.attackLevel) + ": ");
                tempPokemon.defenseLevel = myScan.nextInt();
            }
        }
        return tempPokemon;
